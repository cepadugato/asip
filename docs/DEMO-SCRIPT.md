...
- MCP Watchdog opérationnel (`203.0.113.50:8080`)
...
ssh ansible@203.0.113.50 "grep -E '^(PermitRoot|Password|MaxAuth|Ciphers|MACs)' /etc/ssh/sshd_config"
```
...
ssh ansible@203.0.113.50 "sudo sed -i 's/^PermitRootLogin no/PermitRootLogin yes/' /etc/ssh/sshd_config"
ssh ansible@203.0.113.50 "sudo systemctl restart sshd"
```
...
```
[WATCHDOG] 14:30:15 — DRIFT DETECTED on mcp-watchdog (203.0.113.50)
...
ssh ansible@203.0.113.50 "grep PermitRootLogin /etc/ssh/sshd_config"
...
ssh ansible@203.0.113.50 "cat /var/log/watchdog/audit.json | jq '.[-3:]'"
```
